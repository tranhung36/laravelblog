...
@section('content')
    ...
    @if (Auth::check())
        <div class="pt-15 w-4/5 ml-4/5">
            <a href="/blog/create"
                class="bg-blue-500 uppercase bg-transparent text-gray-100 text-xs font-extrabold py-3 px-5 rounded-3xl">Create
                post</a>
        </div>
    @endif
                ...
                @if (isset(Auth::user()->id) && Auth::user()->id == $post->user_id)
                    <span class="float-right">
                        <form action="/blog/{{ $post->slug }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button class="text-red-500 pl-3 hover:text-red-700" type="submit">
                                Delete
                            </button>
                        </form>
                    </span>
